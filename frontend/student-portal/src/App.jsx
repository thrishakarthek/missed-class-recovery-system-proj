...
function App() {
  ...
  const fetchTodayTimetable = async () => {
    try {
      ...
      const res = await fetch(
        `http://localhost:5000/api/schedules/${classId}/${today}`,
        {
          headers: {
  ...
  const fetchMissedData = async () => {
    try {
      ...
      const res1 = await fetch(
        `http://localhost:5000/api/missed/sessions?classId=${classId}&startDate=${fromDate}&endDate=${toDate}`,
        {
          headers: {
            Authorization: `Bearer ${token}`
          }
      ...
      const res2 = await fetch(
        `http://localhost:5000/api/missed/summary?classId=${classId}&startDate=${fromDate}&endDate=${toDate}`,
        {
          headers: {
            Authorization: `Bearer ${token}`
          }
  ...
  const fetchGeneralNotes = async () => {
    try {
      ...
      const res = await fetch(
        `http://localhost:5000/api/subjects/class/${classId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`
          }
  ...
  const uploadPeerNote = async (sessionId) => {
    try {
      ...
      const res = await fetch("http://localhost:5000/api/sessions/upload-peer-note", {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`
        },
